			...
			switch (apartado) {
				case 1:
					while ((line = br.readLine())!= null) {
                        ...
                        for (int i = 0; i <= 4; i++) {
                       	...
                       	List<I_Jugada> jugadas = Analizador.analizaMano(mano);
				...
				case 2:
					while ((line = br.readLine())!= null) {
						...
						List<I_Jugada> jugadas = Analizador.analizaJugador(jugador, mesa);
				...
				case 3:
					while ((line = br.readLine())!= null) {
						...
						for (int j = 1; j <= numJugadores; j++) {
							Jugador jugador = new Jugador();
							for (int i = 0; i < 2; i++) {
								jugador.setId(datosLinea[j].substring(0, 2));
						...
						List<Jugador> jugadoresOrdenados = Analizador.comparaJugadores(listaJugadores, mesa);
						
                        //Imprimir en el archivo de salida la salida.
						bw.write(line);
						bw.newLine();
						for (Jugador jugador : jugadoresOrdenados) {
							List<I_Jugada> jugadas = jugador.getJugadas();
							lineaSalida = jugador.getId() + ": " + jugadas.get(0).toString();
							bw.write(lineaSalida);
					          bw.newLine();
						}
                        bw.newLine();
